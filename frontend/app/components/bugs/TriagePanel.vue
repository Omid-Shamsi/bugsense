...
const visible = computed(() => actions.value.some((action) => ['begin_review', 'request_information', 'respond_information', 'set_priority', 'set_severity'].includes(action)))
...
watch(() => props.bug, (bug) => {
  ...
  if (bug.allowed_actions.includes('set_priority') || bug.allowed_actions.includes('set_severity')) loadTrackingValues()
}, { immediate: true })
...
<template>
  <section v-if="visible" class="bug-panel workflow-panel">
    ...
    <div v-if="actions.includes('begin_review')" class="workflow-block">
    ...
    <form v-if="actions.includes('request_information')" class="workflow-block workflow-form" @submit.prevent="mutate('request', () => bugs.requestInformation(bug.public_id, requestText), () => { requestText = '' })">
    ...
    <form v-if="actions.includes('respond_information')" class="workflow-block workflow-form" @submit.prevent="mutate('response', () => bugs.respondInformation(bug.public_id, responseText), () => { responseText = '' })">
    ...
    <div v-if="actions.includes('set_priority') || actions.includes('set_severity')" class="classification-grid">
      <form v-if="actions.includes('set_priority')" class="workflow-form" @submit.prevent="mutate('priority', () => bugs.setPriority(bug.public_id, priorityId))">
      ...
      <form v-if="actions.includes('set_severity')" class="workflow-form" @submit.prevent="mutate('severity', () => bugs.setSeverity(bug.public_id, severityId))">
